...
import (
	...
	"optimus-admin/internal/api"
	"optimus-admin/internal/store"
)
...
func TestAuthenticationAndProtectedResources(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]string{"email": "admin@example.com", "password": "Optimus300"})
...
func TestTaskLifecycle(t *testing.T) {
	handler := api.New(store.New(), slog.New(slog.NewTextHandler(io.Discard, nil)))
	authBody := bytes.NewBufferString(`{"email":"admin@example.com","password":"Optimus300"}`)
	signIn := httptest.NewRecorder()
...
func TestAdminCannotAssignOrModifyOwner(t *testing.T) {
	...
	cookie := &http.Cookie{Name: "optimus_session", Value: data.NewSession("usr_aiko")}
...
func TestAPIRejectsAmbiguousJSONAndUntrustedOrigins(t *testing.T) {
	...
	handler.ServeHTTP(trailing, httptest.NewRequest(http.MethodPost, "/api/auth/sign-in", bytes.NewBufferString(`{"email":"admin@example.com","password":"Optimus300"}{}`)))
	if trailing.Code != http.StatusBadRequest {
		t.Fatalf("trailing JSON: want 400, got %d", trailing.Code)
	}

	crossOriginRequest := httptest.NewRequest(http.MethodPost, "/api/auth/sign-in", bytes.NewBufferString(`{"email":"admin@example.com","password":"Optimus300"}`))
