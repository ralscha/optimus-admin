...
export class AdminStore {
  ...
  loadKanban(): Promise<boolean> {
    return this.loadInto(this.kanban, () =>
      this.api.get<KanbanTask[]>("/kanban"),
    );
  }
  loadEvents(): Promise<boolean> {
    return this.loadInto(this.events, () =>
      this.api.get<CalendarEvent[]>("/calendar/events"),
    );
  }
  ...
  loadChats(): Promise<boolean> {
    return this.loadInto(this.conversations, () =>
      this.api.get<Conversation[]>("/chats"),
    );
  }
  loadTransactions(): Promise<boolean> {
    return this.loadInto(this.transactions, () =>
      this.api.get<Transaction[]>("/transactions"),
    );
  }
  loadSettings(): Promise<boolean> {
    return this.loadInto(this.settings, () =>
  ...
  updateMail(
    id: string,
    changes: Partial<Pick<Mail, "read" | "folder">>,
  ): Promise<boolean> {
    return this.saveEntity(this.mails, () =>
      this.api.patch<Mail>(`/mail/${id}`, changes),
    );
  }
  ...
  async saveSettings(settings: Settings): Promise<boolean> {
    const result = await this.execute(() =>
      this.api.put<Settings>("/settings", settings),
    );
    if (!result.ok) return false;
  ...
  async askAi(prompt: string, model: string): Promise<string | null> {
    const result = await this.execute(() =>
      this.api.post<AiReply>("/ai/chat", { prompt, model }),
    );
